...
static func append_json(ranger_dict:Dictionary, filepath:String, folder_name:String = get_directory())->bool:
	...
	print("Attempting to edit file: " + filepath)			
	...
	file.close()	
	print("Successfully appended file: " + filepath)
	return true
...
static func save_json(ranger_dict:Dictionary, folder_name:String = get_directory())->bool:
	...
	while duplicate_filename:
		...
		if not file.file_exists(folder_name+divider+filename+extension):
			duplicate_filename = false
	print("Attempting to save file: " + filename+extension)			
	var result = file.open(folder_name+divider+filename+extension, File.WRITE)
	if result != OK:
		push_error("Failed to save file: " + filename+extension + " in " + folder_name)
		return false
	file.store_string(JSON.print(ranger_dict, "\t"))
	file.close()	
	print("Successfully created file: " + filename+extension + " in " + folder_name)
	return true
...
static func create_directory(dir:Directory, folder:String):
	print("" + folder+" does not exist. Attempting to create folder.")
	...
	for record in collection:
		...
		for key in key_fields:
			if not record.has(key):					
				push_error("Failed to find key " + key + " in record " + str(record_count))
				invalid_record = true
#				
			...
			if record.has(tape_field):								
				for tape_data_field in tape_data_fields:
					if not record[tape_field].has(tape_data_field):
						push_error("Failed to find key " + tape_data_field + " in record " + str(record_count))
						invalid_record = true
#						
					if tape_data_field == "stickers":
						...
						if record[tape_field].has(tape_data_field):
							if not record[tape_field].get(tape_data_field) is Array:
								push_error("[Stickers] is not valid type Array. Record " + str(record_count))
								invalid_record = true									
							else:
								var sticker_count:int = 1
								for sticker in record[tape_field].get(tape_data_field):
									if not sticker is Dictionary and sticker != null:
										push_error("sticker# "+ str(sticker_count) +" in tape# "+ str(tape_count)+" is not valid type Dictionary. Record " + str(record_count))
										invalid_record = true		
									elif sticker != null:
										for sticker_field in sticker_fields:
											if not sticker.has(sticker_field):								
												push_error("Failed to find key " + sticker_field + " in sticker# "+ str(sticker_count) +" in tape# "+ str(tape_count)+ " in record# " + str(record_count))
												invalid_record = true											
									sticker_count+=1									 					
		...
		if tape_count < 3:
			push_error("Not enough tapes. Tape count: " + str(tape_count) + " in record " + str(record_count))
			invalid_record = true
	...
	if tape_snapshot.has("custom_form"):
		if tape_snapshot.custom_form != "":
			...
			if form:									
				...
				print("Retrieved custom form " + tape_snapshot.custom_form)			
...
static func set_custom_monster(tape_snapshot:Dictionary)->Dictionary:
	if tape_snapshot["form"].begins_with("res://mods/") or tape_snapshot["form"].begins_with("res://data/monster_forms/mods_"):
		...
		print("Set fallback form for custom form: " + tape_snapshot.custom_form)
	return tape_snapshot

static func set_char_config(char_config:CharacterConfig, ranger_data, tape1, tape2, tape3, own_tapes:bool = true):	
